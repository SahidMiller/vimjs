...
var configFile = path.resolve(__dirname, "../../base-widget.ini");
var pkgConfigName = "editor-widget";
var baseWidgetOpts = util.parseOpts(rc(pkgConfigName, configFile));
...
function BaseWidget(opts) {
  ...
  util.logger.debug(
    util.typeOf(self),
    "init {" + Object.keys(opts).join(",") + "}"
  );

  self.ready = new Promise((res) => setTimeout(res, 0))
    .then(function () {
...
BaseWidget.prototype.focus = function () {
  if (!this.hasFocus())
    return blessed.Box.prototype.focus.apply(this, arguments);
  return this;
};
...
BaseWidget.prototype.shrinkWidth = function () {
...
BaseWidget.prototype.resolveBinding = function (key, source1, source2, etc) {
  return BaseWidget.resolveBinding.apply(
    this,
    [key, util.callBase(this, BaseWidget, "getBindings")].concat(
...
BaseWidget.resolveBinding = function (key, source1, source2, etc) {
  ...
  for (var name in bindings) {
    if (bindings.hasOwnProperty(name)) {
      ...
      if (
        keyBindings.some(function (binding) {
...
BaseWidget.prototype._initHandlers = function () {
  ...
  self.on("blur", function () {
    util.logger.debug("blur", util.typeOf(self));
  });
