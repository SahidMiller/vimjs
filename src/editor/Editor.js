...
var editorWidgetOpts = util.parseOpts(rc(pkgConfigName, configFile));
...
function Editor(opts) {
  ...
  self.textBuf = new TextBuffer({
    encoding: self.options && self.options.defaultEncoding || "UTF-8",
    text: self.options && self.options.text || "",
  });
  ...
  self._updateContent = lodash.throttle(
    _updateContent,
    self.options.perf && self.options.perf.updateContentThrottle,
    false
  );
  self._updateContent();
}
...
Editor.prototype.open = async function (givenPath) {
  // Handles nonexistent paths
  var self = this;
  await self.ready // ensures textBuf path-changed is triggered in _initHandlers
    .then(function () {
    ...
    .then(function (params) {
      if (params.exists) return self.textBuf.load().then(() => params);
      return params;
    })
    .then(function (params) {
...
Editor.prototype.save = async function (path) {
  var self = this;
  var args = arguments;
  try {
    await (path
...
Editor.getOpenParams = function (givenPath) {
...
Editor.exists = function (givenPath) {
  return fs.promises
    .open(givenPath, "r")
    .then(function (fd) {
      fs.close(fd);
...
Editor.prototype.copy = function () {
  ...
  return new Promise((res, rej) =>
    clipboard.copy(text, (err) => (err ? rej(err) : res()))
  )
    .catch(function (err) {
...
Editor.prototype.paste = function () {
  var self = this;
  return new Promise((res, rej) =>
    clipboard.paste(text, (err) => (err ? rej(err) : res()))
  )
    .catch(function (err) {
