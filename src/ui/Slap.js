...
var { BaseWidget, Editor } = require('../editor');
...
Slap.prototype.open = function (filePath, current) {
  var self = this;
...
Slap.prototype._stopKeyPropagation = function () {
  ...
  var self = this;
  self.lockKeys = true;
  return new Promise((res) => setTimeout(res, 0)).then(function () { self.lockKeys = false; });
};
...
Slap.prototype._initHandlers = function () {
  ...
  self.on('element keypress', function (el, ch, key) {
    ...
    switch (binding) {
      ...
      case 'toggleFileBrowser':
        ...
        return false;
      case 'quit':
        //TODO God willing: quit gracefully
  ...
  util.logger.info("BismAllah", new Error().stack)
  // self.ready.then(self => self._initPlugins());
  util.logger.info("BismAllah1", self.ready)
...
Slap.getUserDir = function () {
  ...
  var userDir = "/.vimjs"
  return new Promise((res, rej) => fs.mkdir(userDir, { recursive: true }, (err) => err ? rej(err) : res())).then(() => {
    return userDir
  });
};
