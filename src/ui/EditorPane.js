...
EditorPane.prototype.save = function (path) {
  ...
  return editor.save(path, process.cwd())
    .then(function (path) { 
      header.message("saved to " + editor.textBuf.getPath(), 'success'); 
      return path
    })
    .catch(function (err) {
...
EditorPane.prototype._initHandlers = function () {
  ...
  self.on('element keypress', function (el, ch, key) {
    switch (self.resolveBinding(key)) {
      case 'save': if (!editor.readOnly()) editor.textBuf.getPath() ? self.save() : self.saveAsForm.show(); return false;
